...
temp <-  rbind(temp0 %>% mutate(Option = "De-intensification"), 
               temp1 %>% mutate(Option = "Low cost"),
               #temp2 %>% mutate(Option = "stand-off pad"),
               temp3 %>% mutate(Option = "High cost"))
temp$Option<-  factor(temp$Option, levels=c("De-intensification","Low cost","High cost")) # Ordering for the legend.
...
tempo <-  rbind(temp0 %>% mutate(Option = "De-intensification"), 
                temp1 %>% mutate(Option = "Low cost"),
                temp2 %>% mutate(Option = "High cost"))#,
#temp3 %>% mutate(Option = "barn"))
tempo$Option<-  factor(tempo$Option, levels=c("De-intensification","Low cost","High cost")) #Ordering for the legend.
...
ggplot(data = temps, mapping = aes(y = profitchange, x = nlosschange, group= Option,colour=Option)) +
  geom_line()+
  geom_point(aes(shape=Option), size=3)+scale_shape_manual(values=c("De-intensification"=16,"Low cost"=17,"High cost"=15))+  
